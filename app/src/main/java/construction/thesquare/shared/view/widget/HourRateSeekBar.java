...
/**
 * Created by gherg on 12/1/2016.
 */

public class HourRateSeekBar extends AppCompatSeekBar {
    private int mThumbSize;
    private TextPaint mTextPaint;
    private double rate;

    public HourRateSeekBar(Context context) {
        this(context, null);
    }

    public HourRateSeekBar(Context context, AttributeSet attrs) {
        this(context, attrs, android.R.attr.seekBarStyle);
    }

    public HourRateSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        rate = getProgress() * 0.5 + 3.5;
        String progressText = "£" + String.valueOf(getProgress() * 0.5 + 3.5);
        ...
        if (getProgress() == 0 || getProgress() == 193) {
            //
        } else {
